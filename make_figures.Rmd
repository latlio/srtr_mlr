...
# full_vars = data.frame(x1 = all_res$cox_1_featsel_$x1)
...
make_perf_df <- function(start) {
  ...
  ci_wide <- map_dfc(ind, function(ind) start %>% pluck(ind) %>% select(-x1))
  ci_long <- ci_wide %>%
...
perf_df$model <- recode(perf_df$model,
                        rfsrc_scale = "Random Survival Forest",
...
feat_selections %>%
  mutate(model = case_when(
    ...
    model == "rfsrc_1_featsel_" ~ "Random Survival Forest",
    model == "ridge_1_featsel_" ~ "Ridge",
    model == "xgbl_1_featsel_" ~ "XGBoost Linear",
    model == "xgbt_1_featsel_" ~ "XGBoost Tree"
  )) %>%
  filter(!model %in% c("Random Survival Forest", "Ridge", "XGBoost Linear"),
...
feat_magnitude_long %>%
  filter(method == "ridge",
         weight != 0,
         str_detect(x1, "(?<![PRE])REC") | 
           str_detect(x1, "?<![XCAN_])INSURANCE") |
           x1 %in% c("MDRDeGFR_B", "VAD")) %>%
  ggplot(aes(x = weight,
             y = reorder(x1, desc(x1)))) + 
